...
@with_goto
def retreat_method(func, x0, a0, d, r, t):
    ...
    if a_cur <= 0:
        a_cur = 0
        goto .step4
    if func(x0 + d * a_pre) <= func(x0 + d * a_cur):
        #转步4
        goto .step4
...
@with_goto
def golden_method(func, x0, a0, b0, d, epsilon, tau=0.618):
...
def test():
    x0 = np.array([-3, -1, -3, -1])
    d0 = np.array([2, 1, 2, 1])
    [a, b] = retreat_method(functions.wood, x0, 0, d0, 0.1, 1.5)
    print(a)
    print(b)
    best_x = golden_method(functions.wood, x0, a, b, d0, epsilon=1e-5)
